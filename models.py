...
class FileImport(models.Model):  # MPR
    ...
    @property
    def loc(self):
        return self.xls_file.path if self.xls_file else self.file_path
                ...
                try:
                    with transaction.atomic():
                        ...
                        if c3:
                            ...
                            if (
                                not pd.isnull(row["Case Number"])
                                and not row["Case Number"] == ""
                            ):
                                ciep3 = ClientIEP.objects.filter(
                                    case_number=str(int(row["Case Number"]))
                                ).first()
                                if ciep3:
                                    c3 = ciep3.client
                            if not c3:
    ...
    def run(self):  # MPR
        ...
        if self.ftype == FileImportTypes.MPR.name:
            (run_report, row_numbers, results) = self.run_MPR()
        elif self.ftype == FileImportTypes.RRIEP.name:
                    ...
                    with transaction.atomic():
                        ...
                        if c3:
                            ...
                            if not pd.isnull(row["Case #"]) and not row["Case #"] == "":
                                ciep3 = ClientIEP.objects.filter(
                                    case_number=str(int(row["Case #"]))
                                ).first()
                                if ciep3:
                                    c3 = ciep3.client
                                    print(
                                        "Client could not be found but there is IEPClient with case number="
                                        + str(ciep3.case_number)
                                    )
                            if not c3:
                                # Create Client
